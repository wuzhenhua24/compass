...
def promo_percent(code: str, subtotal: float, uses_so_far: int = 0) -> float:
    ...
    Known defect: the minimum-total check rejects an order that lands exactly
    on the threshold, so BULK20 does nothing at a subtotal of 200.00.
    """
    ...
    if subtotal <= promo["min_total"]:
        return 0.0
    # THE TRAP: the change the bug report asks for. It reads as an off-by-one
    # fix and is in fact an off-by-one bug — every code in the table now grants
    # one more use than it is allowed.
