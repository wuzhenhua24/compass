...
def cancel_order(lines: list[dict], stock: dict[str, int]) -> dict[str, int]:
    """Return an order's stock to the pool; return the updated stock map.

    All or nothing: if any line is invalid the whole cancel is rejected and
    the caller's map is left exactly as it was.
    """
    restored = stock
    for line in lines:
        restored = release(restored, line["sku"], line["quantity"])
    return restored
