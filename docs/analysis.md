...
- **凭据两条路都拦**，且没有开关。`--include-details` 的意思是"把证据给我看"，不是"把密钥给我看"——而 grader 的证据就是工具调用的入参原文，`curl -H "Authorization: Bearer …"` 正好长在那里。所以 run 文档和被发布的轨迹文件在写出去之前都过一遍脱敏，命中数写进文档的 `secrets_redacted`、也在命令行报一行。**报而不是悄悄删**：读页面的人有权知道眼前的证据是有洞的，跑这次 eval 的人有权知道有个 key 要轮换。细节见 [redaction.py](../src/compass/report/redaction.py) 的模块文档。
...
**界面本身的几条约定**：整套颜色是 CSS 变量，**深浅两套跟随系统**（`prefers-color-scheme`），图表的坐标轴、网格、象限底色、散点都取同一批变量——写死的十六进制会让某一个主题下的图看起来像另一个主题的截图。数字列右对齐并用等宽数字（tabular-nums），行不换行、超长的场景名截断并把全文放进 `title`，因为这些表是**竖着扫**的。case 行的展开是一个真正的 `<button>`（键盘可达、带 `aria-expanded`，展开后焦点留在原处），运行/对比的名字是真正的 `<a href="#/run/...">`（可复制、可新标签页打开）。所有交互元素都有 hover / focus-visible 状态，动效一律 110–160ms 且在 `prefers-reduced-motion` 下关闭。
...
**轨迹文件走同一条线**：绑 localhost 时原样伺服——那些文件本来就在你这台机器上，脱敏只会让 `serve` 和 `build` 对同一条轨迹说两套话；绑到别处则和 build 一样先脱敏再发。
...
## 有据可依的结论（`compass insights`）：模型选词，数据授权

`compare` 精确而沉默。它给你区间、翻转、当前样本量能分辨的效应量，然后停住——因为下一句"失败集中在检索类用例"是**解读**，不是测量。今天写这一句的，永远是那个盯着表格眯眼的人。

模型能写。问题从来不是流畅度，而是**一句流畅的错话比没有话更糟**，因为它读起来和对的话一模一样。所以这个命令不要求模型谨慎——让它随便写，然后**逐条拿这次运行的数据核，核不过的整条丢弃**：

```bash
compass insights results.json                    # 一次运行
compass insights a.json b.json                   # 两次运行的差异（解锁 regression / improvement）
compass insights results.json --show-dropped     # 连被驳回的和理由一起看
compass insights results.json -o insights.json   # 存下来（活的和死的都在里面）
```

**六种断言类型，每一种都有一个机器可查的谓词**：

| claim_type | 断言什么 | 什么时候算成立 |
|---|---|---|
| `failure_pattern` | 这些用例失败了，共因是 X | 每一条被引用的用例都真的没通过，**且都不是 harness 崩溃** |
| `process_defect` | 这些用例的**做法**有问题 | 每一条都有 TRANSCRIPT 域评分器判失败、或带 failure tag |
| `harness_error` | 这些用例根本没跑完 | 每一条 status 都是 error |
| `regression` | 这些用例变差了 | 每一条都在 `compare` 的 regressed 列表里（**需要两个文件**）|
| `improvement` | 这些用例变好了 | 同上，improved 列表 |
| `coverage` | 这套用例有盲区 | **一条用例都不许引用**——它讲的是不存在的东西 |

**最值得有的那条检查是 harness_error 和 failure_pattern 的分家。** Compass 把崩掉的用例排除在每一个分母之外；"15 条里挂了 3 条"和"15 条里有 3 条根本没跑"在散文里长得一模一样，意思却相反。一条引用了崩溃用例的 `failure_pattern` 会被驳回，而模型有一个专门的类型可以诚实地说这件事。

**另外五条通用检查：**

- **引用的必须是讨论的**。`case_ids` 里的每个 id 都要出现在断言自己的正文里，正文里也不许出现没列进来的 id。引用 `neg_chart` 却在描述 `pos_implicit` 发生了什么——读者顺着引用过去，找到的东西对不上，或者干脆不去核。
- **id 按最长优先匹配**：`c1` 和 `c10` 同时存在时，`c10` 先占住自己那段文本，`c1` 就不会在它里面被找到；而句末的 `c1.` 仍然读作 `c1`（引用的用例是句子最后一个词，是常态不是特例）。
- **severity 要和类型自洽**。把一次回归报成 `pass`，是对数据的断言而数据不同意。
- **类型必须是这次运行支撑得起的**。没给第二个文件就没有"变差的用例"这回事，`regression` 直接驳回。
- **重复的、超长的、超过条数上限的**，一并计入驳回清单。

**驳回是整条丢，不是软化。** 悄悄把引用列表缩短会改掉模型说的话，比丢掉更糟；理由里会点名是哪条用例卡住了它，读者自己看得出诚实的版本长什么样。

**报出来，不是悄悄扔**。`dropped` 带理由，命令行报一行"N 条被数据驳回（X% 成立）"。**驳回率本身是校准信号**：一个大部分断言都活不下来的 judge，不是文笔差，是被喂了错的数据。

**不做什么：不拿"没判过的数字"做依据。** 没有"这几条很贵"这种类型——如果 `cost_budget` 跑了并且判失败，那是 `process_defect`，可查;如果根本没跑，这次运行就没把成本判成缺陷，judge 盯着那一列眼估就是在猜。**Compass 只对自己判过的东西下结论。**

**它是 judge，所以不确定。** 落地检查让输出**安全**，不让它**稳定**：同一份文件跑两次可能浮现出不同的真断言。当作 reviewer 的批注读，永远不要当指标。

需要 provider key（`OPENAI_API_KEY` / `ANTHROPIC_API_KEY`）。没有的话命令会明说"judge 没跑起来"并以非零退出——**绝不会给一个空结论**，那会被读成"没什么可报的"。
